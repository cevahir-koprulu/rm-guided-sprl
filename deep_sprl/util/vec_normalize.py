...
class VecNormalize(VecEnvWrapper):
    ...
    def set_venv(self, venv):
        ...
        if self.context_bounds is None:
            ...
            if len(self.observation_space.shape) != 1 or \
                    self.obs_rms.mean.shape[0] != self.observation_space.shape[0] - self.context_bounds[0].shape[0]:
                raise ValueError("venv is incompatible with current statistics.")
        self.ret = np.zeros(self.num_envs)
    ...
    def step_wait(self):
        ...
        if self.training:
            if self.dim_off is None:
                ...
                self.obs_rms.update(obs[:, 0:self.dim_off])
    ...
    def normalize_obs(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.norm_obs:
            if self.dim_off is None:
                ...
                obs_sub = np.clip(
                    (obs[:, 0: self.dim_off] - self.obs_rms.mean) / np.sqrt(self.obs_rms.var + self.epsilon),
                    -self.clip_obs, self.clip_obs)
                obs = np.concatenate((obs_sub, self.normalize_context(obs[:, self.dim_off:])), axis=-1)
        return obs

    def normalize_context(self, contexts):
        b = 0.5 * (self.context_bounds[1] + self.context_bounds[0])
        m = 0.5 * (self.context_bounds[1] - self.context_bounds[0])
        return (contexts - b[None, :]) / m[None, :]
