...
class GaussianSampler:
    ...
    def __init__(self, mean, variance, bounds, seed=0):
        np.random.seed(seed)
        self.seed = seed
        self.bounds = bounds
        self._mean = mean
    ...
    def set_statistics(self, mean, variance):
        self._mean = mean
        self.covariance = variance if isinstance(variance, np.ndarray) else variance
...
class UniformSampler:
    ...
    def __init__(self, lower_bound, upper_bound, seed=0):
        np.random.seed(seed)
        self.seed = seed
