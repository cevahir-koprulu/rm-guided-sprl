...
class AbstractSelfPacedTeacher:
    ...
    def _compute_expected_performance(self, dist, rm_transitions_buffer, rm_transition_contexts, old_c_log_prob_t_dict):
        ...
        for context_subset in rm_transitions_buffer["transitions"]:
            print(f"context_subset: {context_subset}")
            print(rm_transitions_buffer["transitions"][context_subset]["rewards"])
            if len(context_subset) != 0:
                ...
                weighted_return += torch.sum(importance_weight * rm_transitions_buffer["transitions"][context_subset]["rewards"])
            else:
                weighted_return += torch.sum(rm_transitions_buffer["transitions"][context_subset]["rewards"])
        return weighted_return / rm_transitions_buffer["num_trajectories"]
                                           


class RMguidedSelfPacedTeacherV2(AbstractTeacher, AbstractSelfPacedTeacher):
    ...
    def __init__(self, target_log_likelihood, target_sampler, initial_mean, initial_variance, context_bounds, perf_lb,
        ...
        if isinstance(initial_variance, np.ndarray):
            ...
            flat_init_chol = GaussianTorchDistribution.flatten_matrix(initial_variance * np.eye(self.context_dim), tril=False)

        super(RMguidedSelfPacedTeacherV2, self).__init__(initial_mean, flat_init_chol, target_log_likelihood, target_sampler,
                                                 max_kl, context_bounds, callback=callback)
    ...
    def expected_performance(self, x, rm_transitions_buffer, rm_transition_contexts, old_c_log_prob_t_dict, 
                             obj=True, grad=False, negate=False):
        dist = GaussianTorchDistribution.from_weights(self.context_dim, x, dtype=torch.float64)
        perf = self._compute_expected_performance(dist, rm_transitions_buffer, rm_transition_contexts, old_c_log_prob_t_dict)
        if negate:
            perf = -1 * perf
    ...
    def update_distribution(self, contexts, values, rm_transitions_buffer):
        ...
        for context_subset in rm_transitions_buffer["transitions"]:
            rm_transitions_buffer["transitions"][context_subset]["rewards"] = \
                to_float_tensor(rm_transitions_buffer["transitions"][context_subset]["rewards"],
        ...
        perf_constraint = NonlinearConstraint(
            lambda x: self.expected_performance(x, rm_transitions_buffer, rm_transition_contexts, old_c_log_prob_t_dict),
            self.perf_lb, np.inf,
            jac=lambda x: self.expected_performance(x, rm_transitions_buffer, rm_transition_contexts, old_c_log_prob_t_dict,
                                                    obj=False, grad=True),
            keep_feasible=False)
        ...
        try:
            ...
            # even once
                ...
                # Define the objective plus Jacobian
                objective = lambda x: self.expected_performance(x, rm_transitions_buffer, rm_transition_contexts, 
                                                                old_c_log_prob_t_dict, grad=True, negate=True)
                res = minimize(objective, x0, method="trust-constr", jac=True, bounds=bounds,
