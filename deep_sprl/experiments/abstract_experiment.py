...
from deep_sprl.teachers.spl import SelfPacedTeacher, SelfPacedTeacherV2, RMguidedSelfPacedTeacher, RMguidedSelfPacedTeacherV2
...
class CurriculumType(Enum):
    ...
    def __str__(self):
        ...
        elif self.rm_guided_self_paced_v2():
            return "rm_guided_self_paced_v2"
    ...
    def rm_guided_self_paced_v2(self):
        return self.value == CurriculumType.RMguidedSelfPacedV2.value
    ...
    def from_string(string):
        ...
        elif string == str(CurriculumType.RMguidedSelfPacedV2):
            return CurriculumType.RMguidedSelfPacedV2
...
class ExperimentCallback:
    ...
    def __init__(self, log_directory, learner, env_wrapper, save_interval=5, step_divider=1):
        ...
        if self.env_wrapper.teacher is not None:
            if isinstance(self.env_wrapper.teacher, SelfPacedTeacher) or \
                isinstance(self.env_wrapper.teacher, SelfPacedTeacherV2) or \
                    isinstance(self.env_wrapper.teacher, RMguidedSelfPacedTeacher) or \
                        isinstance(self.env_wrapper.teacher, RMguidedSelfPacedTeacherV2):
...
class AbstractExperiment(ABC):
    APPENDIX_KEYS = {"default": ["DISCOUNT_FACTOR", "STEPS_PER_ITER", "LAM", "PCMDP"],
                     CurriculumType.SelfPaced: ["ALPHA_OFFSET", "KL_EPS", "OFFSET", "ZETA"],
                     CurriculumType.SelfPacedV2: ["DELTA", "KL_EPS"],
                     CurriculumType.RMguidedSelfPaced: ["ALPHA_OFFSET", "KL_EPS", "OFFSET", "ZETA"],
                     CurriculumType.RMguidedSelfPacedV2: ["DELTA", "KL_EPS"],
    ...
    def process_parameters(self):
        allowed_overrides = {"DISCOUNT_FACTOR": float, "ZETA": float, "ALPHA_OFFSET": int, "OFFSET": int, 
                             "STEPS_PER_ITER": int, "LAM": float, "AG_P_RAND": float, "AG_FIT_RATE": int,
                             ...
                             "TARGET_TYPE": str, "KL_EPS": float, "PCMDP": bool,                       
        }
    ...
    def evaluate(self):
        ...
        if (self.curriculum.self_paced() or self.curriculum.self_paced_v2() or \
            self.curriculum.rm_guided_self_paced() or self.curriculum.rm_guided_self_paced_v2()) and \
