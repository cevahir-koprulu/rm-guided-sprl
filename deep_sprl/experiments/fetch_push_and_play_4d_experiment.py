...
class FetchPushAndPlay4DExperiment(AbstractExperiment):
    TARGET_TYPE = "narrow"
    TARGET_MEAN = np.array([-0.14, -0.14, 0.14, 0.14])
    TARGET_VARIANCES = {
        "narrow": np.diag(np.square([1e-3, 1e-3, 1e-3, 1e-3])),
        "wide": np.diag(np.square([0.025, 0.025, 0.025, 0.025])),
    }

    PCMDP = True

    LOWER_CONTEXT_BOUNDS = np.array([-0.15, -0.15, 0.05, 0.05])
    UPPER_CONTEXT_BOUNDS = np.array([-0.05, -0.05, 0.15, 0.15])
    ...
    INITIAL_MEAN = np.array([-0.05, -0.05, 0.05, 0.05])
    INITIAL_VARIANCE = np.diag(np.square([.01, .01, .01, .01]))

    STD_LOWER_BOUND = np.array([1e-3, 1e-3, 1e-3, 1e-3])
    KL_THRESHOLD = 8000.
    ...
    def create_environment(self, evaluation=False):
        if self.PCMDP:
            env = gym.make("ContextualFetchPushAndPlay4DProduct-v1")
        else:
            env = gym.make("ContextualFetchPushAndPlay4D-v1")
    ...
    def create_learner_params(self):
        return dict(common=dict(gamma=self.DISCOUNT_FACTOR,
                    ...
                    ppo=dict(n_steps=self.STEPS_PER_ITER,
                             ...
                             max_grad_norm=None,
                             batch_size=128,
                             learning_rate=0.0003,  # self.LEARNING_RATE,  # 0.000025,
                             ),
                    sac=dict(learning_rate=0.0003,  # self.LEARNING_RATE,
                             buffer_size=500000, # 250000 # self.SAC_BUFFER,
    ...
    def get_env_name(self):
        return f"fetch_push_and_play_4d_{self.TARGET_TYPE}"
