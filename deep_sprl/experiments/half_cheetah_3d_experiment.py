...
class HalfCheetah3DExperiment(AbstractExperiment):
    ...
    TARGET_VARIANCES = {
        ...
        "wide": np.diag(np.square([1., 1., 1.])),
    }

    PCMDP = True

    LOWER_CONTEXT_BOUNDS = np.array([.5, 2., 3.5])
    ...
    INITIAL_MEAN = np.array([.5, 2., 4.])
    INITIAL_VARIANCE = np.diag(np.square([.5, .5, .5]))
    ...
    def get_env_name(self):
        return f"half_cheetah_3d_{self.TARGET_TYPE}"
