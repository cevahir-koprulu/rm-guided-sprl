...
class TwoDoorDiscrete4DExperiment(AbstractExperiment):
    ...
    TARGET_VARIANCES = {
        "narrow": np.diag(np.square([4e-3, 4e-3, 4e-3, 4e-3])),
        "wide": np.diag(np.square([1., 1., 1., 1.])),
    }
    ...
    INITIAL_VARIANCE = np.diag(np.square([.5, .5, .5, .5]))
    ...
    def create_learner_params(self):
        return dict(common=dict(gamma=self.DISCOUNT_FACTOR,
                                ...
                                verbose=0,
                                device=self.device,
                                policy_kwargs=dict(net_arch=[64, 64], # [256, 256]
                    ...
                    sac=dict(learning_rate=0.0003,  # self.LEARNING_RATE,
                             buffer_size=60000,  # self.SAC_BUFFER,
                             learning_starts=500,
                             batch_size=64,
                             train_freq=5,
                             target_entropy="auto"))
    ...
    def get_env_name(self):
        return f"two_door_discrete_4d_{self.TARGET_TYPE}"
